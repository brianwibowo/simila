                ...
                <div class="card shadow-sm border-0 mb-4">
                    <div class="card-body">
                        <div class="d-flex justify-content-between align-items-center mb-4">
                            ...
                            @if(isset($fromValidation) && $fromValidation)
                                <a href="{{ route('waka-kurikulum-list-validasi') }}" class="btn btn-secondary d-flex align-items-center">
                                    <i class="bi bi-arrow-left me-2"></i> Kembali
                                </a>
                            @else
                                <a href="{{ route('waka-kurikulum-list-diajukan') }}" class="btn btn-secondary d-flex align-items-center">
                                    <i class="bi bi-arrow-left me-2"></i> Kembali
                                </a>
                            @endif
                        </div>
                                    ...
                                    <div class="card-body">
                                        ...
                                        <table class="table table-bordered">
                                            <tbody>
                                                ...
                                                <tr>
                                                    @if(isset($fromValidation) && $fromValidation)
                                                        <th>Perusahaan Pengaju</th>
                                                        <td>
                                                            @if(isset($pengirimPerusahaan) && $pengirimPerusahaan)
                                                                {{ $pengirimPerusahaan->name }}
                                                            @else
                                            ...
                                            @if(isset($fromValidation) && $fromValidation)
                                                @if($kurikulum->validasi_sekolah == 'proses')
                                                    <form action="{{ route('waka-kurikulum-setuju', $kurikulum) }}" method="POST">
                                                        ...
                                                        <button type="submit" class="btn btn-success w-100 mb-1">
                                                            <i class="bi bi-check-lg me-1"></i> Setujui Kurikulum
                                                        </button>
                                                    </form>
                                                    <button type="button" 
                                                ...
                                                @else
                                                    <button class="btn btn-secondary" disabled>
                                                        <i class="bi bi-lock me-1"></i> Sudah Divalidasi
                                                    </button>
                                                @endif
                                            @else
                                                @if($kurikulum->validasi_sekolah != 'disetujui' || $kurikulum->validasi_perusahaan != 'disetujui')
                                                    <a href="{{ route('waka-kurikulum-edit', $kurikulum) }}" class="btn btn-warning">
                                                        <i class="bi bi-pencil me-1"></i> Edit Kurikulum
                                                    </a>
                                                    <form action="{{ route('waka-kurikulum-destroy', $kurikulum) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus kurikulum ini?')">
                                                        @csrf
                                                        @method('DELETE')
