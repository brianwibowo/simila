        ...
        <div>
            ...
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-0">
                    <li class="breadcrumb-item"><a href="{{ route('perusahaan-beasiswa-index') }}">Daftar Batch</a></li>
                    <li class="breadcrumb-item active" aria-current="page">{{ $pendaftar->nama_siswa }}</li>
                </ol>
            </nav>
        </div>
        <a href="{{ route('perusahaan-beasiswa-show', $beasiswa->id) }}" class="btn btn-secondary shadow-sm">
                ...
                <div class="card-body">
                    <ul class="list-group list-group-flush">
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            ...
                            <a href="{{ Storage::url($pendaftar->raport) }}" class="btn btn-sm btn-outline-primary" target="_blank">
                                <i class="fas fa-download me-1"></i>Lihat
                            </a>
                        </li>
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            <div><i class="fas fa-file-signature text-success me-2"></i>Surat Rekomendasi</div>
                            <a href="{{ Storage::url($pendaftar->surat_rekomendasi) }}" class="btn btn-sm btn-outline-success" target="_blank">
                                <i class="fas fa-download me-1"></i>Lihat
                            </a>
                        </li>
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            <div><i class="fas fa-file-signature text-warning me-2"></i>Surat Motivasi</div>
                            <a href="{{ Storage::url($pendaftar->surat_motivasi) }}" class="btn btn-sm btn-outline-warning" target="_blank">
                                <i class="fas fa-download me-1"></i>Lihat
                            </a>
                        </li>
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            <div><i class="fas fa-briefcase text-info me-2"></i>Portofolio</div>
                            <a href="{{ Storage::url($pendaftar->portofolio) }}" class="btn btn-sm btn-outline-info" target="_blank">
                                <i class="fas fa-download me-1"></i>Lihat
                            </a>
                        </li>
                    </ul>
        ...
        <div class="col-lg-4">
            <div class="card shadow-sm">
                ...
                <div class="card-body">
                    ...
                    <form action="{{ route('perusahaan-beasiswa-seleksi', $pendaftar->id) }}" method="POST">
                        @csrf
                        <div class="mb-3">
                            <select name="status" class="form-select" required>
                                <option value="pending" @if($pendaftar->status == 'pending') selected @endif>Menunggu Review</option>
                                <option value="diterima" @if($pendaftar->status == 'diterima') selected @endif>Diterima</option>
                                <option value="ditolak" @if($pendaftar->status == 'ditolak') selected @endif>Ditolak</option>
                            </select>
                        </div>
                        <div class="d-grid">
