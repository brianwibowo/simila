...
@section('content')
<div class="container">
    <h2>Tambah Soal untuk Ujian: {{ $certificationExam->nama_ujian }}</h2> {{-- Nama variabel disesuaikan --}}

    <div class="card">
        <div class="card-header">Form Tambah Soal</div>
        <div class="card-body">
            <form action="{{ route('perusahaan-sertifikasi-questions.store', $certificationExam->id) }}" method="POST"> {{-- Nama variabel disesuaikan --}}
                @csrf
                <div class="mb-3">
                ...
                <div class="mb-3">
                    ...
                    <select class="form-control @error('jawaban_benar') is-invalid @enderror" id="jawaban_benar" name="jawaban_benar" required>
                        ...
                        <option value="1" {{ old('jawaban_benar') == '1' ? 'selected' : '' }}>Pilihan 1 (A)</option>
                        <option value="2" {{ old('jawaban_benar') == '2' ? 'selected' : '' }}>Pilihan 2 (B)</option>
                        <option value="3" {{ old('jawaban_benar') == '3' ? 'selected' : '' }}>Pilihan 3 (C)</option>
                        <option value="4" {{ old('jawaban_benar') == '4' ? 'selected' : '' }}>Pilihan 4 (D)</option>
                    </select>
                    @error('jawaban_benar')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                ...
                <div class="mb-3">
                    ...
                    <input type="number" class="form-control @error('nilai_akhir') is-invalid @enderror" id="nilai_akhir" name="nilai_akhir" value="{{ old('nilai_akhir') }}" required min="0">
                    @error('nilai_akhir')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
